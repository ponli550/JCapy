...
## 🛰️ TUI Architecture Specifications
...
- **Sidebar**: Fixed width (26 chars), contains:
  ...
  - Control Buttons: `⚙️ Manage`, `🔍 Find`, `🚪 Quit`
...
- **🔌 MCP Servers**: Dynamic list of active protocols.
- **🧩 Workspace Elements**: Registry for all installed Widgets and Skills (Plugins).
- **🎨 Themes**: One-click switching via `ThemeItem` gallery.
### 4. v2.0 Architectural Directives
- **🕹️ Modal Input Engine**: Formalized the requirement for a state machine to handle **NORMAL**, **INSERT**, and **VISUAL** modes in the core client.
- **🧩 Command Grammar**: Added the "Verb + Noun" composability as a core performance metric.
- **⚡ Async Core Benchmarks**: Updated the technology stack to prioritize non-blocking job control (via Textual `@work`) and byte-stream output for GPU-accelerated terminals.
